...
#define TYPE_OPERAND 3  // complex double  
...
// Worker function for each GPU thread
void gpu_worker(int device_id) {
    // 1. Set the active device for this specific thread
    hipError_t status = hipSetDevice(device_id);
    if (status != hipSuccess) {
        std::cerr << "Failed to set device " << device_id << std::endl;
        return;
    }

    // 2. Initialize hipBLAS handle for this device
    hipblasHandle_t handle;
    hipblasCreate(&handle);

    // Placeholder: You will add your Vector splitting, hipMalloc, 
    // and hipblasSgemm calls here.
    std::cout << "Thread managing Device " << device_id << " is ready." << std::endl;

    // 3. Cleanup (after GEMM is finished)
...
int main() {
    int num_devices = 0;
    hipGetDeviceCount(&num_devices); // Query total GPUs

    if (num_devices < 2) {
        std::cerr << "This sample requires at least 2 GPUs." << std::endl;
        return 1;
    }
    ...
    for (int i = 0; i < num_devices; ++i) {
        threads.emplace_back(gpu_worker, i);
    }

    // Join all threads to synchronize with the host
    for (auto& t : threads) {
        t.join();
    }

    std::cout << "All GPU operations completed." << std::endl;
    return 0;
}

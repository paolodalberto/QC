 ...
 */
__global__ void permuteColumns(int N, int M, const ZC* d_input, 
                               const int* d_perm_indices, ZC* d_output) {
    ...
    if (new_col_idx < M) {
        ...
        if (threadIdx.x == 0) { 
            const ZC* src = d_input + old_col_idx * N;
            ZC* dst = d_output + new_col_idx * N;
...
int main() {
    int N = 3; // rows
    int M = 3; // columns
    size_t matrix_size_bytes = N * M * sizeof(ZC);
    ...
    std::vector<ZC> h_sort_keys = { h_A[0], h_A[N], h_A[2*N] };
    
    for (ZC e : h_sort_keys)  std::cout << e << " \n";
    
    // 3. Allocate all necessary device memory
    ZC *d_A, *d_B, *d_sort_keys;
    int* d_perm_indices;
    HIP_CHECK(hipMalloc(&d_A, matrix_size_bytes));
    HIP_CHECK(hipMalloc(&d_B, matrix_size_bytes)); // Output matrix
    HIP_CHECK(hipMalloc(&d_sort_keys, M * sizeof(ZC))); // Keys on GPU
    ...
    HIP_CHECK(hipMemcpy(d_sort_keys, h_sort_keys.data(), M * sizeof(ZC), hipMemcpyHostToDevice));
